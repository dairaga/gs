...
func Fold[T, R any](t gs.Try[T], z R, op funcs.Func[T, R]) R {
	return funcs.BuildOrElse(t.Fetch, z, op)
}

func Collect[T, R any](t gs.Try[T], p funcs.Try[T, R]) gs.Try[R] {
	return funcs.Build(t.Fetch, funcs.TryRecover(p, From[R]), gs.Failure[R])
}

func FlatMap[T, R any](t gs.Try[T], op funcs.Func[T, gs.Try[R]]) gs.Try[R] {
	return funcs.Build(t.Fetch, op, gs.Failure[R])
}

func Map[T, R any](t gs.Try[T], op funcs.Func[T, R]) gs.Try[R] {
	return funcs.Build(t.Fetch, funcs.AndThen(op, gs.Success[R]), gs.Failure[R])
}

func TryMap[T, R any](t gs.Try[T], op funcs.Try[T, R]) gs.Try[R] {
	return funcs.Build(t.Fetch, funcs.TryRecover(op, From[R]), gs.Failure[R])
}

func CanMap[T, R any](t gs.Try[T], op funcs.Can[T, R]) gs.Try[R] {
	return funcs.Build(t.Fetch, funcs.CanTransform(op, FromWithBool[R]), gs.Failure[R])
}
